...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function atualizarTabela() {
        ...
        row.insertCell(0).textContent = mecanicos_certificacao[0];
        row.insertCell(1).textContent = mecanicos_certificacao[1];
        row.insertCell(2).textContent = mecanicos_certificacao[2];
        row.insertCell(3).textContent = mecanicos_certificacao[3] === 1 ? 'Sim' : (mecanicos_certificacao[3] === 0 ? 'Não' : 'Sem certificação');
         });
        })
